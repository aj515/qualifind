-- QualiFind: full schema, RLS policies, signup trigger, and seed data.
-- Run this once in the Supabase SQL Editor (Project > SQL Editor > New query > paste > Run).
--
-- Normalized entities: providers, students/admins, documents, program_documents,
-- application_steps, saved_programs, eligibility_results (per-student, computed
-- client-side and persisted — see src/lib/eligibility.js), student_step_progress.
...
-- The frontend's public /register form always passes role: 'student' — admin
-- accounts are never self-service (see README "Creating an admin account").
-- =========================================================================
...
-- =========================================================================
-- SAVED_PROGRAMS — student bookmarks with an application-progress status.
-- Status mostly auto-derives client-side from student_step_progress (see
-- ActionPlanPage.jsx) but stays independently settable for programs with no
-- steps, or steps that happen on the provider's end and can't be self-checked.
-- =========================================================================
create table public.saved_programs (
  student_id uuid references auth.users(id) on delete cascade,
  program_id uuid references public.programs(id) on delete cascade,
  status text default 'saved' check (status in ('saved', 'in_progress', 'applied', 'withdrawn')),
...
-- =========================================================================
-- STUDENT_STEP_PROGRESS — which application_steps a student has checked off.
-- =========================================================================
...
-- Documents (13 unique, deduped across all 8 programs' requirement checklists)
...
-- Application steps (3 per program)
