...
export default function StudentLayout() {
  ...
  return (
    <div className="bg-paper font-sans text-ink antialiased bg-dot-grid min-h-screen">
      {/* Desktop Sidebar — always visible, no collapse/toggle. On mobile the bottom
          tab bar covers navigation instead (see below). */}
      <aside className="hidden lg:flex fixed left-0 top-0 h-full w-72 bg-card z-50 flex-col border-r-2 border-ink">
        <div className="h-20 flex items-center px-6 border-b-2 border-ink">
          <Link to="/dashboard" className="flex items-center gap-3 no-underline group cursor-pointer">
      ...
      <div className="lg:pl-72 flex flex-col min-h-screen">
        <header className="fixed top-0 left-0 lg:left-72 right-0 h-20 bg-paper/95 backdrop-blur-md z-40 border-b-2 border-ink flex items-center justify-between px-4 lg:px-8">
          ...
          <div className="flex items-center gap-3 ml-4">
            ...
            {/* Sign Out — mobile only; desktop has it in the sidebar footer. */}
            <button
              onClick={signOut}
              className="lg:hidden w-10 h-10 flex items-center justify-center rounded-full bg-card border-2 border-ink shadow-pop-sm hover:bg-accent-pink hover:text-white transition-all"
              title="Sign out"
              aria-label="Sign out"
            >
